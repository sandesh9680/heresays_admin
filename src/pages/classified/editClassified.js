...
const EditClassified = ({ inputs, title, text, name }) => {
  ...
  const getdata = () => {
    axios.get(`${ApiUrl}getClassifiedById/${params.id}`).then((res) => {
      let requiredData = res.data.list[0];
      setPublishStatus(requiredData.published_at?"unpublish":'publish')
      let classifiedData = requiredData.textfield;
      setDefaultData(classifiedData);
    });
  };
  ...
  const onSubmit = async () => {
    ...
    if (params.op == "edit") {
      axios
        .put(`${ApiUrl}updateClassified/${params.id}`, {
          label: "test",
          textfield:textData
        })
        .then((res) => {
          ...
          setSaveAction(true);
          setIsLoading(false);
          alert("Your Classified has been updated successfully")
        })
        .catch((err) => {
          console.log("err", err);
          setIsLoading(false);
      ...
      axios
        .post(`${ApiUrl}addClassified`, {
          label: "test",
          textfield:textData
        })
        .then((res) => {
          navigate("/app/classified");
          setSaveAction(true);
          setIsLoading(false);
          alert("Your Classified has been added successfully")
        })
        .catch((err) => {
          console.log("err", err);
          setIsLoading(false);
  ...
  function sayHello() {
    ...
    axios
      .put(`${ApiUrl}updateClassifiedStatus/${params.id}`, {
        type: publishStatus,
      })
      .then((res) => {
        setIsLoading(false);
